...
The types `POST /api/v1/monitors` accepts are the `enum` on `MonitorInput.type` in `/api/v1/openapi.json`, generated from the same list the route enforces. Anything else — invented, or a type the UI can create but this API cannot configure — is refused with `400`. The three [Web3 types](web3-monitoring.md) are in that list; they are EVM JSON-RPC, not a generic chain call. The [`llm` type](llm-monitoring.md) is in it too, though its API key is not: that field is UI-only, so a monitor created here must reach an endpoint needing no key, or one whose key a human already set. MQTT, gRPC and the rest stay in the UI.
