...
**Settings → Export / Import** as an administrator to export or import a
`.ugbackup` file.
...
2. Open **Settings → Export / Import** on the target instance.
...
[Backing up and restoring](../backup-and-restore.md). Configuration export/import
does not replace that procedure.

Configuration export/import is intentionally unavailable through `/api/v1`, MCP,
or agent skills. See the [REST API reference](rest-api.md#export-and-import-are-not-rest-apis)
for the boundary.
