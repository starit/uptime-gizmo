...
An empty RPC quantity (`0x`) is a failed read, not a balance of zero. ERC-20
reads require a 32-byte ABI word. A well-formed `0x0`, or a balance read
against a block more than 15 minutes old, is confirmed against a fallback
network before it fails the minimum, so a lagging member of a pooled RPC
endpoint can't alert on its own — whether it answers with a wrong zero or a
plausible-looking but out-of-date amount; it still fails once every
configured network agrees, or when there is no fallback to ask.
