...
reads require a 32-byte ABI word. A well-formed `0x0` is confirmed against a
fallback network before it fails the minimum, so a lagging member of a pooled
RPC endpoint can't alert on its own; it still fails once every configured
network agrees, or when there is no fallback to ask.
